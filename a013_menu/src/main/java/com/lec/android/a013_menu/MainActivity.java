...
public class MainActivity extends AppCompatActivity {
    ...
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {   // menu 리소스(xml) 로 부터 Menu 생성

        getMenuInflater().inflate(R.menu.menu_main1, menu);
        Log.d("myapp", "onCreateOptionsMenu - 최초 메뉴키를 눌렀을떄 호출됨");
        ...
        return true; //false 리턴하면 메뉴는 보여지지 않느다.
    }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        // 메뉴가 보여질때마다 '로그인', '로그아웃' 이 enable/ disable 토글되어 보이기
            if(bLog){   // 로그인 상태
            // getItem(index) index 번쨰 MenuItem 객체 리턴
