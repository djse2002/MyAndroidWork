...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 방법3
        BackThread3 thread3 = new BackThread3(handler3);  // 메인스레드의 Handler 객체를 외부 클레스에 넘겨줌
        thread3.setDaemon(true);
        thread3.start();

        // 방법4
        BackThread4 thread4 = new BackThread4(handler4);  // 메인스레드의 Handler 객체를 외부 클레스에 넘겨줌
    ...
    class  BackThread1 extends Thread{
        ...
        public void run() {
            super.run();
            while (true){
                // 방법1.) 메인에서 생성된 Handler 객체의 sendEmptyMessage 를 통한 Message 전달
                //작업 스레드
                backValue1++;
...
// Message 를 수신합니다.
    ...
    // 방법3)
    Handler handler3 = new Handler(){  //inner class 생성
...
class  BackThread3 extends Thread{
    ...
    BackThread3(Handler handler) {this.handler = handler;}   //handler 외부에서 받아와서 this로 사용
    ...
    public void run() {
        ...
        while(true){
            ...
            handler.sendMessage(msg);  // 메인스레드의 핸들러에 메세지 보내기 생성할때 만들어진 handler
