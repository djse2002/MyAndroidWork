...
;            super.onPreExecute();
        ...
        // 백그라운드 작업  (※ 반드시 구현!!!)
        @Override
        protected Integer doInBackground(Integer... integers) {  // 가변 매개변수 integers 는 Integer[] 이다.
       ...
       // 백그라운드 작업 도중(여러번) 호출 가능. 진행상황 업데이트, 중간중간 UI 업데이트시 사용
        ...
        protected void onProgressUpdate(Integer... values) {
            Log.d("myapp", "Progress : " + values[0] + "%");  //  values는 publishProgress(i) 가 보낸 값
            super.onProgressUpdate(values);

            tvBackValue1.setText("onProgressUpdate:" + values[0]);   //★ Handler 없이도 메인 UI 접근 가능!
        }

        // doInBackground() 완료 되면 호출
        @Override
