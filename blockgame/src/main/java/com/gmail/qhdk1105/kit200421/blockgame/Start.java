package com.gmail.qhdk1105.kit200421.blockgame;
...
public class Start extends AppCompatActivity implements View.OnClickListener {
            ...
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    if(time >= 0) {
                        ...
                        if(time > 0 ){
                            time --; // 1초 감소하고 1초후에 다시 Runnable 수행(run 수행)
    ...
    public void onClick(View v) {
        ...
        switch (imageView.getId()){
            // 맨 아래 블럭 iv[7] I mageView 의 색상과 일치하는 버튼인지 판정
            case R.id.ivRed:   // 빨강버튼 클릭시

                if("0".equals(iv[7].getTag().toString())) isOk = true; // 빨강블럭의 tag 값 = "0"
                break;
            case R.id.ivGreen: // 초록버튼 클릭시
                if("1".equals(iv[7].getTag().toString())) isOk = true; // 초록블럭의 tag 값 = "0"
                break;
            case R.id.ivBlue:  // 파랑버튼 클릭시
                if("2".equals(iv[7].getTag().toString())) isOk = true; // 파랑블럭의 tag 값 = "0"
                break;
        } // end switch
        ...
        if(isOk){ // 버튼 색깔을 맞췄다면!
            ...
            // 가장 위으이 블럭(iv[0]) ImageView 는 랜덤으로 생성
